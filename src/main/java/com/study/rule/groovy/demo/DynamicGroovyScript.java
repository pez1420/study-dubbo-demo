package com.study.rule.groovy.demo;

/**
 * https://my.oschina.net/skymozn/blog/833838
 *
 * @author pez1420@gmail.com
 * @version $Id: DynamicGroovyScript.java v 0.1 2020/7/8 2:18 下午 pez1420 Exp $$
 */
...
public class DynamicGroovyScript {
    ...
    public static void main(String[] args) throws ScriptException {
        ...
        while (iterator.hasNext()) {
            ...
            if (script == null || script.replaceAll("\\s+", "").length() == 0)
                continue;
    ...
    private DataObject getRuleset() {
        ...
        ruleSet.xput("规则集.评级.学费档次",
            "tuitionFee>=60000?'A':(tuitionFee>=40000&&tuitionFee<60000?'B':(tuitionFee>=20000&&tuitionFee<40000)?'C':'D')");
    ...
    private DataObject[] getCourses() {
        DataObject[] courses = new DataObject[] { new DataObject(), new DataObject(),
                                                  new DataObject(), new DataObject(),
                                                  new DataObject(), new DataObject(), };
    ...
    private String maxCourseHours() {
        ...
        execScript.append("    student.courses.each { maxHour = Math.max(maxHour,it.classHour)}")
            .append("\n");
        execScript.append("    student.courses.find({it.classHour == maxHour}) //默认最后一句为返回值")
            .append("\n");
        execScript.append("})()").append("\n");
