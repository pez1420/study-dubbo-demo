...
import com.study.rule.groovy.ruleengine.manager.GroovyManager;
...
public class GrovvyCondition extends AbstractCondition {
    ...
    public GrovvyCondition(String script) {
        ...
        if (script != null && script.trim().length() != 0) {
            this.groovyClass = GroovyManager.getInstance().getGroovyClassLoader()
                .parseClass(script);
        }
    }
    ...
    public void run(Context context) throws UnitRunException {
        try {
            ...
            this.result = (Boolean) GroovyManager.getInstance().invokeMethod(this.groovyClass,
                "run", context);
