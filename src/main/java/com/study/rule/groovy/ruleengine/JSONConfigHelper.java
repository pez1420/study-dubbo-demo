...
import com.study.rule.groovy.ruleengine.action.Action;
import com.study.rule.groovy.ruleengine.condition.Condition;
...
class JSONConfigHelper {
    ...
    private static Unit parseLoop(JSONObject jo) throws ClassNotFoundException,
        ...
        if (jo != null) {
            ...
            if ("action".equals(type)) {
                ...
                action.registerUnit(
                    parseLoop(jo.getJSONObject(Constants.JSON_FIELDNAME_ACTION_UNIT)));
                unit = action;
                ...
                condition.registerTrueUnit(
                    parseLoop(jo.getJSONObject(Constants.JSON_FIELDNAME_CONDITION_TRUEUNIT)));
                condition.registerFalseUnit(
                    parseLoop(jo.getJSONObject(Constants.JSON_FIELDNAME_CONDITION_FALSEUNIT)));
                unit = condition;
            } else {
                //TODO 无法识别的类型
            }
     ...
     * @return 入口单元
     * */
    public static Unit parse(String json) throws IllegalAccessException, InstantiationException,
